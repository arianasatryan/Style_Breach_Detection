...
def computeMeasures(inputText, groundTruthData, producedData):
    ...
    (groundTruthWordPositions, producedDataWordPositions, totalWordCount) = getWordPositionsFromCharacterPositions(inputText, groundTruthData["positions"], producedData["positions"])
...
def main(inputDataset, inputRun, outputDir):
    ...
    for file in os.listdir(inputDataset):
        if file.endswith(".txt"):
            ...
            with open(truthFileName, 'r', encoding='utf-8') as truthFile:
                #print("%s" % ("processing " + filePrefix))
                producedTruthFilename = inputRun + "/" + filePrefix + ".truth"
                if os.path.exists(producedTruthFilename):
                    with open(producedTruthFilename,'r', encoding='utf-8') as producedTruthFile:
    ...
    with open(outputDir + "/" + evaluationOutputFileName, 'w', encoding='utf-8') as outFile:
